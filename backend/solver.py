...
def empty_system_initiate(total_len,len3):
    matrix = np.zeros((total_len + len3 + 1,total_len))
    extra_column = np.zeros((total_len + len3 + 1,1))
...
def solve(add1,add2,sum):
    ...
    if check_combination(add1,add2,sum):
        ...
        matrix, extra_column = empty_system_initiate(total_len,len(sum))
